...
def upload_image_path(instance, filename):
    print(instance)
    print(filename)
    new_filename = random.randint(1,39102393240)
    name, ext = get_filename_ext(filename)
    final_filename = '{new_filename}{ext}'.format(new_filename=new_filename, ext=ext)
    return "books/{new_filename}/{final_filename}".format(new_filename=new_filename,  final_filename=final_filename )
...
class Book(models.Model):
    ...
    def less_quantity(self):
        return self.copies < .1 * self.total_copies
    @property
    def name(self):
        return self.title
